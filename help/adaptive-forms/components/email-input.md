...
- **必須** - コンポーネントをアダプティブフォームに表示する場合は、このオプションを選択します。 オプションを選択した後、フォームの送信を続行する前に値を入力する必要があります。 **コンポーネントを非表示** または **コンポーネントを無効にする**  （内） **基本** 」タブ（このオプションが選択されている場合）
...
<br>
    ...
    - **パターン** — このオプションを使用すると、E メールで許可する検証パターンを入力できます。 ここでは正規表現も使用できます。
