...
チェックボックスは通常、小さな四角形で表され、クリックまたはタップしてオン/オフを切り替えることができます。 チェックボックスをオンにすると、関連するオプションまたは機能がアクティブまたは有効であることを示すチェックマークが表示されます。
...
- **使いやすさ**：チェックボックスは視覚的に明確で、バイナリ選択を簡単におこなう方法を提供します。 直感的で分かりやすく、使いやすくなっています。
...
- **エラー回避**：チェックボックスを使用すると、フォームを送信する前に選択内容を確認し確認できるので、エラーの確率が低下します。

## バージョンと互換性 {#version-and-compatibility}

アダプティブFormsチェックボックスコアコンポーネントが、コアコンポーネント 2.0.52 の一部としてリリースされました。次の表に、サポートされているすべてのバージョン、AEMの互換性、および対応するドキュメントへのリンクを示します。

|  |  |
|---|---|
| コンポーネントのバージョン | AEM as a Cloud Service |
| --- | --- |
| v1 | <br>[リリース 2.0.52](/help/versions.md) 以降と互換性あり | 互換性あり | 互換性あり |

コアコンポーネントのバージョンとリリースについて詳しくは、[コアコンポーネントのバージョン](/help/versions.md)ドキュメントをご覧ください。

## 技術的詳細 {#technical-details}

アダプティブFormsチェックボックスコアコンポーネントの最新情報については、 [GitHub](https://github.com/adobe/aem-core-forms-components/tree/master/ui.af.apps/src/main/content/jcr_root/apps/core/fd/components/form/checkbox/v1/checkbox). コアコンポーネントの開発について詳しくは、 [コアコンポーネント開発者向けのドキュメント](/help/developing/overview.md)をご覧ください。

## 設定ダイアログ {#configure-dialog}

設定ダイアログを使用すると、訪問者に対してチェックボックスエクスペリエンスを簡単にカスタマイズできます。 また、シームレスなユーザーエクスペリエンスを実現するために、簡単にチェックボックスオプションを定義できます。

![「基本」タブ](/help/adaptive-forms/assets/checkbox-basic.png)

- **名前** - フォームコンポーネントは、フォーム内とルールエディター内の両方で一意の名前で簡単に識別できますが、名前にスペースや特殊文字を含めることはできません。

- **タイトル**  — タイトルを使用すると、フォーム内のコンポーネントを簡単に識別でき、デフォルトでは、タイトルはコンポーネントの横に表示されます。 タイトルを追加しない場合、コンポーネントは表示されません。
...
- **送信された値のデータタイプ**：このオプションは、いずれかのオプションが選択された場合に送信される値のデータタイプを指定します。 「**送信された値のデータタイプ**」が「`Number`」に指定されている場合に、「**オプション** 」タブの「**データ値**」に文字列データを追加すると、画面に `Value type mismatch` エラーメッセージが表示されます。
...
- **オンにすると、戻り値**  — チェックボックスがオンまたは選択されたときに、チェックボックスに関連付ける値を指定するには、このオプションを選択します。 これは、チェックボックスをマークまたはチェックしたときに発生するアクションです。
- **「オフ」を有効にします。** — 以前にチェックされていたチェックボックスをオフにする機能を有効または無効にするには、このオプションを選択します。
   - 次の場合 **オフを有効にする** が有効または true に設定されている場合、ユーザーは自由にチェックボックスのオンとオフを切り替えることができます。 必要に応じて、チェックボックスのオン/オフを切り替えることができます。

   - 次の場合 **オフを有効にする** が無効または false に設定されている場合、チェックボックスがオンになると、ユーザーはチェックを解除できなくなります。
- **オフの場合、戻り値** - 「 」オプションを使用すると、チェックボックスがオフまたはオフの状態の場合に、チェックボックスに関連付ける値を指定できます。
...
デザインダイアログは、チェックボックスコンポーネントの CSS スタイルを定義および管理するために使用されます。

### 「スタイル」タブ {#styles-tab}

アダプティブFormsチェックボックスコアコンポーネントは、AEMをサポートしています [スタイルシステム](/help/get-started/authoring.md#component-styling).
