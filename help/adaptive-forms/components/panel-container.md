...
- **フォーム送信時に子コンポーネントのデータをグループ化する（オブジェクト内でデータをラップする）**  — このオプションを選択すると、子コンポーネントのデータが親コンポーネントの JSON オブジェクト内にネストされます。 ただし、このオプションが選択されていない場合、送信された JSON データはフラット構造で、親コンポーネントのオブジェクトはありません。 例：

   - このオプションを選択すると、子コンポーネント（通り、市区町村、郵便番号など）のデータが、JSON オブジェクトとして親コンポーネント（住所）内にネストされます。 これにより階層構造が作成され、データは親コンポーネントの下に整理されます。
   ...
   - このオプションを選択しない場合、送信される JSON データはフラット構造を持ち、親コンポーネント (Address) のオブジェクトを持ちません。 すべてのデータは同じレベルにあり、階層的な組織はありません。
