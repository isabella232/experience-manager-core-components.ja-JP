...
アダプティブFormsでは、 [送信ボタン](/help/adaptive-forms/components/submit-button.md) および [リセットボタン](/help/adaptive-forms/components/reset-button.md)を使用すると、ユーザーは便利にフォームを送信またはリセットできます。 ボタンコンポーネントは、特定のニーズに基づいて、これらのアクションを実行するように柔軟に設定できます。
