---
title: アダプティブFormsコアコンポーネント — 切り替えコンポーネント
description: アダプティブForms Switch コアコンポーネントの使用またはカスタマイズ。
...
# 切り替えコンポーネント{#switch-adaptive-forms-core-component}

切り替えコンポーネントは、2 つのオプションから選択できるグラフィカルユーザーインターフェイスです。 通常は 2 状態の切り替えで、機能、設定、機能の有効/無効を切り替えることができます。 スイッチコンポーネントは、現在の状態を視覚的に表し、特定の機能のオン/オフを表示するように設計されています。

スイッチコンポーネントは、値を true または false に設定するブール型の制御要素です。 たとえば、サウンドのミュートやミュート解除、Bluetooth や WiFi の有効/無効を切り替える場合に使用します。

![切り替えコンポーネントの例](/help/adaptive-forms/assets/switch-example.png)

## 使用方法 {#reasons-to-use-switch}

アダプティブフォームでスイッチを使用する一般的な理由は次のとおりです。

- **ユーザーインタラクション**：ユーザーは、切り替えコンポーネントをクリックまたはタップして操作できます。

- **州（米国）**：スイッチコンポーネントには、オンとオフの 2 つの状態があります。 スイッチコンポーネントの初期状態は、デフォルトの設定や、制御するフィーチャの現在の状態によって異なります。

- **視覚表現**：スイッチコンポーネントは、色や位置を変更することで、現在の状態を視覚的に反映します。

- **コントロール機能**：切り替えコンポーネントは、AEM Form の特定の機能を有効または無効にするために使用されます。 例えば、ユーザーは機能のオン/オフを切り替えることができます。

## バージョンと互換性 {#version-and-compatibility}

アダプティブForms Switch コアコンポーネントは、コアコンポーネント 2.0.64 の一部としてリリースされました。次の表に、サポートされているすべてのバージョン、AEMの互換性、および対応するドキュメントへのリンクを示します。
...
アダプティブFormsスイッチのコアコンポーネントに関する最新情報については、 [GitHub](https://github.com/adobe/aem-core-forms-components/tree/master/ui.af.apps/src/main/content/jcr_root/apps/core/fd/components/form/switch/v1/switch). コアコンポーネントの開発について詳しくは、 [コアコンポーネント開発者向けのドキュメント](/help/developing/overview.md)をご覧ください。

## 設定ダイアログ {#configure-dialog}

設定ダイアログを使用すると、訪問者に対してスイッチコンポーネントのエクスペリエンスを簡単にカスタマイズできます。 また、シームレスなユーザーエクスペリエンスを実現するために、スイッチコンポーネントのオプションを簡単に定義することもできます。
...
- **状態の値を保持解除**  — このオプションを選択すると、切り替えコンポーネントが選択されていない場合に返される値を指定できます。
- **オプション**  — 各オプションのデータ値とテキストを表示します。
   - **データ値**  — アダプティブフォームで切り替えが有効になったときに送信される値を指定します。
   - **表示テキスト**  — アダプティブフォームで切り替えが有効な場合にラベルとして表示するテキストを指定します。
   - **オフデータ値**  — アダプティブフォームで切り替えが有効になっていない場合に送信する値を指定します。 このオプションは、 **状態の値を保持解除** スイッチが有効になっている。
   - **表示テキストをオフ**  — アダプティブフォームで切り替えが有効になっていない場合にラベルとして表示するテキストを指定します。 このオプションは、 **状態の値を保持解除** スイッチが有効になっている。
...
- **ラベルを非表示にする**  — 切り替えコンポーネントのラベルを非表示にするには、このオプションを選択します。

- **ラベルを表示**  — スイッチコンポーネントのラベルを表示するには、このオプションを選択します。

## デザインダイアログ {#design-dialog}

デザインダイアログは、切り替えコンポーネントの CSS スタイルを定義および管理するために使用します。

### 「スタイル」タブ {#styles-design-tab}

アダプティブForms Switch コアコンポーネントは、AEMをサポートしています。 [スタイルシステム](/help/get-started/authoring.md#component-styling).

![デザインダイアログ](/help/adaptive-forms/assets/checkbox-style.png)

- **デフォルトの CSS クラス**：アダプティブFormsスイッチグループコアコンポーネントのデフォルト CSS クラスを指定できます。
