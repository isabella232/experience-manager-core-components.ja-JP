...
| `bundle-unversioned-packages` | OSGi バンドルが Export-Package 宣言を持つバージョンと、Import-Package 宣言を持つバージョン範囲を指定しているかどうかを確認します。 次のようなエラーが発生します。 <p> </p> `[ERROR] org.acme:mybundle:0.0.1-SNAPSHOT: Bundle org.acme:mybundle:0.0.1-SNAPSHOT is exporting package org.acme.foo without a version.`<p> </p>トラブルシューティングをおこなうには、必ず `package-info.java` を、エクスポートするバージョンを指定するパッケージに追加します。 | はい | はい |
