...
| `sdkFormsVersion` | `latest` | 条件 `aemVersion=cloud` および `includeFormsenrollment=y` または `includeFormscommunications=y`に設定すると、Forms SDK のバージョンを指定できます ( 例： `2020.12.17.02`) をクリックします。 |
...
| `includeFormsheadless` | `n` | 次を含む [Forms Core Components](https://github.com/adobe/aem-core-forms-components) 依存関係、 `ui.frontend.react.forms.af`、およびヘッドレスアーティファクト。 |

## システム要件 {#requirements}

| アーキタイプ | AEM as a Cloud Service | AEM 6.5 | Java SE | Maven |
|---------|---------|---------|---------|---------|
| [39](https://github.com/adobe/aem-project-archetype/releases/tag/aem-project-archetype-39) | 継続的 | 6.5.7.0+ | 8、11 | 3.3.9 以上 |
