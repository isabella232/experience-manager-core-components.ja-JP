...
>[!TIP]
>
>最新のAEMプロジェクトのアーキタ [イプはGitHubで確認できます](https://github.com/adobe/aem-project-archetype)。
...
* サンプルモデル、サーベルト、フィルター、スケジューラーを含むバンドルの例
* ユニット、統合、およびクライアントサイドのテスト
* ReactまたはAngularでのSPA実装の例（オプション）
...
* WKNDチュートリアル — アーキタイプの活用方法を含むAEMの開発に関する優れた入門は、「 [](https://docs.adobe.com/content/help/en/experience-manager-learn/getting-started-wknd-tutorial-develop/overview.html) AEM Sites - WKND Tutorials」を参照して、単純なプロジェクトの実装に使用するアーキタイプの実例を示します。
* WKNDイベントチュートリアル — AEMでのシングルページアプリ(SPA)の開発に特に関心がある場合は、専用の [WKNDイベントチュートリアルを必ずご覧ください](https://helpx.adobe.com/experience-manager/kt/sites/using/getting-started-spa-wknd-tutorial-develop.html)。
* ダウンロードして自分で開始！  — 以下の簡単な手順に従うと、GitHubで利用可能な現在のプロジェクトのアーキタイプを簡単にダウンロードし、最初のプ [ロジェクトを作成することができます](#how-to-use-the-archetype)。
...
| `appID` |  | コンポーネント、設定、コンテンツのフォルダーとCSS IDに使用されるアプリケーションID |
...
| `frontendModule` | なし | 専用のフロントエンドモジュールを含めます(、、、 `none`のい [`general`](uifrontend.md)ずれか [`angular`](uifrontend-angular.md)を含みま [`react`](uifrontend-react.md)す) |
| `singleCountry` | y | サンプルコンテンツで言語マスター構造を作成する |
| `includeDispatcherConfig` | n | Adobe Managed Services用のプロジェクトの作成時に、 <br> AEM用のプロジェクトをクラウドサービスセットとして作成する場合に、プロジェクトセットに対してディスパッ [`cloud`](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/dispatcher.cloud) チャー設定を生成する [](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/landing/home.html)<br>[`ams`](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/dispatcher.ams) かどうかを定義します。 |
...
>[!NOTE]
>Windows上で実行し、ディスパッチャー設定を生成する場合は、管理者特権のコマンドプロンプトまたはLinux用のWindowsサブシステムで実行する必要があります( [問題329](https://github.com/adobe/aem-project-archetype/issues/329)を参照)。
...
| `autoInstallPackage` | ui.contentとui.appsのコンテンツパッケージを、content-package-maven-pluginと共にパッケージマネージャーにインストールし、localhostのデフォルトの作成者インスタンス（ポート4502）を設定します。 Hostname and port can be changed with the `aem.host` and `aem.port` user defined properties. |
