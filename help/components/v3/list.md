---
title: リストコンポーネント（v3）
description: コアコンポーネントのリストコンポーネント（v3）を使用すれば、静的リストや静的リストを簡単に作成できます。
...
リストコンポーネント（v3）では、子ページの動的リストや、任意で定義した項目の静的リストなどを作成できます。[デザインダイアログ](#design-dialog)では、利用可能なリストの種類や書式設定のオプションをテンプレート作成者が定義できます。[編集ダイアログ](#edit-dialog)では、利用可能なリストタイプやリスト要素の書式設定方法の中から、コンテンツエディターが選択できます。
