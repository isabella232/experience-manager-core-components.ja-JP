...
このドキュメントでは、ティーザーコンポーネントの現在のバージョン (2022 年 2 月にコアコンポーネントのリリース2.18.0で導入された v2) について説明します。
...
* **リンク**  — このファイルは、コンテンツページ、外部 URL、ページアンカーにリンクします。
* **リンクを新しいタブで開く**  — 有効にした場合、リンクは新しいブラウザータブで開きます。
* **コールトゥアクション**  — このオプションを使用すると、複数の宛先へのリンクを許可できます。
   * 最初のコールトゥアクションでリンクされたページは、ティーザーのタイトル、説明、画像を継承する場合に使用されます。
...
* **ページからアイキャッチ画像を継承**  — リンクされたページのページプロパティで定義された画像を使用します。何も見つからない場合は、現在のページを使用します。
* **画像アセット**  — からアセットをドロップ [アセットブラウザー](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/sites/authoring/fundamentals/environment-tools.html?lang=ja) または **参照** ローカルファイルシステムからアップロードするオプション。
...
* **アクセシビリティ用代替テキスト**  — このフィールドでは、視覚に障害のあるユーザー向けの画像の説明を定義できます。
   * **ページから代替テキストを継承**  — このオプションでは、 `dc:description` DAM のメタデータ、またはアセットがリンクされていない場合は現在のページのメタデータ。
* **代替テキストを指定しない**  — このオプションは、画像が単なる装飾画像である場合や、ページに追加情報を一切伝えない場合に、スクリーンリーダーなどの支援テクノロジーで画像を無視するようにマークします。
...
ティーザーコンポーネントは、AEMをサポートしています [スタイルシステム。](/help/get-started/authoring.md#component-styling).
...
* **デフォルトのタイトルタイプ**  — ティーザーのタイトルで使用する H タグを定義します。
