...
埋め込み可能リソースを使用すると、埋め込みリソースをさらにカスタマイズできます。パラメーター化したり、追加情報を含めたりすることができます。作成者は、事前設定済みの信頼できる埋め込みデータから選択できます。コンポーネントにはYouTubeが埋め込まれ、すぐに使用できます。
...
* **幅** - 埋め込みビデオの幅
* **高さ** - 埋め込みビデオの高さ
* **ミュートを有効にする**  — このパラメーターは、ビデオをデフォルトでミュートして再生するかどうかを指定します。これを有効にすると、自動再生が最新のブラウザーで機能する可能性が高くなります。
* **自動再生を有効にする**  — このパラメーターは、プレイヤーの読み込み時に最初のビデオを自動的に開始するかどうかを指定します。これは、発行インスタンスで、またはオーサリングインスタンスで&#x200B;**表示を発行済み**&#x200B;として使用する場合にのみ有効です。
* **ループを有効にする** - 1つのビデオの場合、このパラメーターはプレイヤーが最初のビデオを繰り返し再生するかどうかを指定します。プレイリストの場合、プレイヤーはプレイリスト全体を再生し、最初のビデオで開始を再生します。
* **インライン再生を有効にする(iOS)**  — このパラメーターは、iOSのHTML5プレーヤーでビデオをインライン（オン）またはフルスクリーン（オフ）のどちらで再生するかを制御します。
* **無制限の関連ビデオ**  — このオプションを無効にすると、関連ビデオは再生されたばかりのビデオと同じチャネルから取得され、それ以外の場合は任意のチャネルから取得されます。

「有効にする」オプションは、[デザインダイアログ](#design-dialog)を通じて有効にする必要があり、デフォルト値として設定できます。
...
ただし、埋め込みコンポーネントはAEMのグローバルHTML AntiSamy衛生フレームワークフィルタリングルールセットに従うので、ルールはより複雑です。これは`/libs/cq/xssprotection/config.xml`にあります。 必要に応じ、開発者によってプロジェクト固有の設定に対応するよう、これをオーバーレイすることができます。
...
### 埋め込み可能なタイプタブ{#embeddable-types-tab}
...
* **Allowed Embeddables**  - Embeddableoptionがアクティブな場合に、コンテンツ作成者が使用できる埋め込み可能なプロセッサーを定義する複数選択 **** します。

### YouTubeタブ{#youtube-tab}

![埋め込みコンポーネントのデザインダイアログの「YouTube」タブ](/help/assets/embed-design-youtube.png)

* **ミュート動作の設定を許可** - YouTubeの埋め込みタイプが選択されている場合に **、コンテンツ作成者がコンポーネントの「ミュートを** 有効にする」オプションを設定できます
   * **デフォルト値のミュート** - YouTubeの埋め込みタイプが選択されている場合に、 **「ミュートを有効にする」** オプションが自動的に設定されます
* **自動再生動作の設定を許可** - YouTubeの埋め込みタイプが選択されている場合に **、コンテンツ作成者がコンポーネントの「自動再生を** 有効にする」オプションを設定できます
   * **デフォルト値のautoplay** - YouTubeの埋め込みタイプが選択されている場合に、「自動再生を **有効にする」** オプションが自動的に設定されます
* **ループ動作の設定を許可** - YouTubeの埋め込みタイプが選択されている場合に **、コンテンツ作成者がコンポーネントの「読み込みを** 有効にする」オプションを設定できます
   * **loopのデフォルト値** - YouTubeの埋め込みタイプが選択されている場合、自動的に「読み込みを **有効にする」** を設定します
* **インライン再生の設定を許可(iOS)** - YouTubeの埋め込みタイプが選択されている場合に、コンテンツ作成者がコンポーネントの「インライン再生を **有効にする(iOS)** 」オプションを設定できます
   * **インライン再生(iOS)のデフォルト値** - YouTubeの埋め込みタイプが選択されている場合に、「インライン再生を **有効にする(iOS)** 」オプションを自動的に設定
* **インラインビデオの設定を許可** - YouTubeの埋め込みタイプが選択されている場合に、コンテンツ作成者がコンポーネントの「 **無制限関連** ビデオ」オプションを設定できます
   * **無制限の関連ビデオのデフォルト値** - YouTubeの埋め込みタイプが選択されている場合 **、「無制限の関連ビデオ** 」オプションが自動的に設定されます
